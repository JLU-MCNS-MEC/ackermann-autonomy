...
def generate_launch_description() -> LaunchDescription:
    ...
    bringup_share = get_package_share_directory('ackermann_simulation')
    integration_share = get_package_share_directory('ackermann_bringup')
    ...
    default_world = os.path.join(
        bringup_share, 'worlds', 'waypoint_obstacle.sdf'
    )
    ...
    control_boundary = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(os.path.join(
            integration_share, 'launch', 'control_boundary.launch.py')),
        launch_arguments={'use_sim_time': 'true'}.items(),
    )
    chassis_adapter = Node(
        package='ackermann_autonomy',
