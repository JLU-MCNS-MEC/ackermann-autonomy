...
from ackermann_autonomy.nav2_waypoint_sender import (
    default_waypoint_file,
    poses_from_waypoints,
    wait_for_map,
)
from ackermann_autonomy.waypoint_tracker_node import Waypoint
import pytest
import rclpy
...
def test_default_waypoint_file_is_packaged() -> None:
    assert default_waypoint_file().endswith(
        'ackermann_autonomy/config/nav2_trajectory.csv'
    )
